    ...
    pool.getConnection((err, connection) => {
      ...
      connection.query(getMonthIdsSql, [months], (err, monthRows) => {
        if (err) {
          ...
          return callback({
            success: false,
            message: "Error fetching month IDs",
          });
        }
        const monthIds = monthRows.map((row) => row.month_id);
        monthIds.forEach((month_id) => {
          const checkBudgetSql =
            "SELECT * FROM Budgets WHERE month_id = ? AND user_id = ? AND budget_criteria = ?";
          connection.query(
            checkBudgetSql,
            [month_id, user_id, budgetCriteria],
            (err, budgetRows) => {
              if (err) {
                connection.release();
                return callback({
              ...
              if (budgetRows.length > 0) {
                const updateBudgetSql =
                  "UPDATE Budgets SET amount = ? WHERE month_id = ? AND user_id = ? AND budget_criteria = ?";
                connection.query(
                  updateBudgetSql,
                  [amount, month_id, user_id, budgetCriteria],
                  (err, result) => {
                    if (err) {
                      connection.release();
                      return callback({
                        success: false,
                        message: "Error updating budget for specific month",
                      });
                    }
                  }
                );
                ...
                const insertBudgetSql =
                  "INSERT INTO Budgets (month_id, user_id, budget_criteria, amount) VALUES (?, ?, ?, ?)";
                connection.query(
                  insertBudgetSql,
                  [month_id, user_id, budgetCriteria, amount],
                  (err, result) => {
                    if (err) {
                      connection.release();
                      return callback({
                        success: false,
                        message: "Error inserting budget for specific month",
                      });
                    }
                    addExpenseByDefault(
                      token,
        ...
        return callback({
          success: true,
          message: "Budgets updated or inserted successfully",
        });
      });
    });
  });
  ...
  authFunctions.getUserIdFromToken(token, (result) => {
    ...
    pool.getConnection((err, connection) => {
      if (err) {
        return callback({
          success: false,
          message: "Error acquiring database connection",
        });
      }
      ...
      connection.query(getMonthIdSql, [month], (err, monthRows) => {
        ...
        const getBudgetCriteriaIdSql =
          "SELECT budget_id FROM Budgets WHERE budget_criteria = ?";
        connection.query(
          getBudgetCriteriaIdSql,
          [budgetCriteria],
          (err, budgetRows) => {
            if (err || budgetRows.length === 0) {
              ...
              return callback({
                success: false,
                message: "Error fetching budget criteria",
              });
            }
            ...
            const selectSql =
              "SELECT * FROM Expenses WHERE user_id = ? AND month_id = ? AND budget_criteria_id = ?";
            connection.query(
              selectSql,
              [user_id, month_id, budgetCriteria_id],
              (err, rows) => {
                if (err) {
                  connection.release();
                  return callback({
                ...
                if (rows.length > 0) {
                  ...
                  const updateSql =
                    "UPDATE Expenses SET spent_amount = ? WHERE user_id = ? AND month_id = ? AND budget_criteria_id = ?";
                  connection.query(
                    updateSql,
                    [updatedSpentAmount, user_id, month_id, budgetCriteria_id],
                    (err, result) => {
                      if (err) {
                        connection.release();
                        return callback({
                  ...
                  const insertSql =
                    "INSERT INTO Expenses (user_id, month_id, budget_criteria_id, spent_amount) VALUES (?, ?, ?, ?)";
                  connection.query(
                    insertSql,
                    [user_id, month_id, budgetCriteria_id, expense],
                    (err, result) => {
                      if (err) {
                        connection.release();
                        return callback({
    ...
    pool.getConnection((err, connection) => {
      ...
      connection.query(getMonthIdSql, [monthName], (err, monthRows) => {
        if (err || monthRows.length === 0) {
          ...
          return callback({
            success: false,
            message: "Error fetching month ID",
          });
        }
    ...
    pool.getConnection((err, connection) => {
      ...
      connection.query(
        ...
        (err, budgetRows) => {
          ...
          connection.query(
            insertExpenseSql,
            [user_id, month_id, budgetCriteria_id, 0],
            (err, result) => {
              connection.release();
              if (err) {
                return callback({
                  success: false,
                  message: "Error adding default expense",
                });
              }
              return callback({
                success: true,
